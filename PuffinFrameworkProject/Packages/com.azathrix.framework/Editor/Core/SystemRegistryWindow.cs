...
namespace ParaCrossGames.Framework.Editor.Core
{
    ...
    public class SystemRegistryWindow : EditorWindow
    {
        ...
        private void DrawToolbar()
        {
            ...
            _showOnlyDisabled = GUILayout.Toggle(_showOnlyDisabled, "仅禁用", EditorStyles.toolbarButton, GUILayout.Width(60));
            _showOnlyEnabled = GUILayout.Toggle(_showOnlyEnabled, "仅启用", EditorStyles.toolbarButton, GUILayout.Width(60));

            GUILayout.FlexibleSpace();

            if (GUILayout.Button("重新扫描", EditorStyles.toolbarButton, GUILayout.Width(70)))
            {
                ScanSystems();
            }
        ...
        private void DrawSystemList()
        {
            ...
            EditorGUILayout.LabelField($"总计: {_scannedSystems.Count}", GUILayout.Width(70));
            EditorGUILayout.LabelField($"启用: {enabledCount}", GUILayout.Width(70));
            EditorGUILayout.LabelField($"禁用: {disabledCount}", GUILayout.Width(70));
            EditorGUILayout.EndHorizontal();
        ...
        private void DrawModuleGroup(string groupName, List<ScannedSystemInfo> systems)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(16);
            DrawHeaderCell("启用", _colEnabled, 0);
            DrawHeaderCell("系统名称", _colName, 1);
            DrawHeaderCell("优先级", _colPriority, 2);
            DrawHeaderCell("依赖", _colDependencies, -1);
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
