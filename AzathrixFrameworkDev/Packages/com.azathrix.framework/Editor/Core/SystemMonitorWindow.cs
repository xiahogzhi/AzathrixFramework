...
namespace ParaCrossGames.Framework.Editor.Core
{
    ...
    public class SystemMonitorWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (!Application.isPlaying)
            {
                EditorGUILayout.HelpBox("编辑器模式 - 仅显示已注册系统，性能数据需在 Play 模式下查看", MessageType.Info);
            }
        ...
        private void DrawToolbar()
        {
            ...
            _showDisabled = GUILayout.Toggle(_showDisabled, "显示禁用", EditorStyles.toolbarButton, GUILayout.Width(70));
        ...
        private void DrawSystemList()
        {
            ...
            EditorGUILayout.LabelField($"系统数: {list.Count}", GUILayout.Width(80));
            EditorGUILayout.LabelField($"启用: {list.Count(s => s.IsEnabled)}", GUILayout.Width(70));
            EditorGUILayout.LabelField($"总耗时: {totalMs:F2}ms", GUILayout.Width(110));
            EditorGUILayout.EndHorizontal();
        ...
        private void DrawModuleGroup(string groupName, List<SystemStatus> systems)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(16);
            DrawHeaderCell("状态", _colStatus, 0);
            DrawHeaderCell("系统名称", _colName, 1);
            DrawHeaderCell("优先级", _colPriority, 2);
            DrawHeaderCell("上次(ms)", _colLastMs, 3);
            DrawHeaderCell("平均(ms)", _colAvgMs, 4);
            GUILayout.Label("操作", GUILayout.Width(_colAction));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
