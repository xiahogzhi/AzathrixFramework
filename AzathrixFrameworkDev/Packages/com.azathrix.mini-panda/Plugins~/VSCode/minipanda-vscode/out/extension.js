...
function activate(context) {
    ...
    context.subscriptions.push(vscode.debug.registerDebugAdapterDescriptorFactory('minipanda', factory));
    ...
    // 注册命令
    context.subscriptions.push(vscode.commands.registerCommand('minipanda.restartLanguageServer', () => {
...
function restartLanguageClient(context) {
    if (retryTimer) {
        clearTimeout(retryTimer);
        retryTimer = null;
    }
...
class MiniPandaDebugAdapterFactory {
    createDebugAdapterDescriptor(session, executable) {
        return new vscode.DebugAdapterInlineImplementation(new debugAdapter_1.MiniPandaDebugSession());
    }
}
class MiniPandaConfigurationProvider {
