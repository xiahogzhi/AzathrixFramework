...
namespace  Azathrix.UpmEditor.Editor.UI
{
    ...
    public class UPMPublishWindow : EditorWindow
    {
        ...
        private void DrawActions()
        {
            ...
            // Pack
            GUI.enabled = canSign;
            if (GUILayout.Button(new GUIContent("打包 (.tgz)", "使用 Unity 打包带签名的 tgz 文件"), GUILayout.Height(28)))
            {
                PackWithSignature();
            }
            ...
            if (GUILayout.Button(new GUIContent("打包并发布", "打包签名后发布到 Registry"), GUILayout.Height(28)))
            {
